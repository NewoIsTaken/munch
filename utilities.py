...
def get_menu(location, meal, date=datetime.now(ZoneInfo("America/New_York"))):
...
def lunch_time(start=time(11, 30), end=time(16, 30), now=datetime.now(ZoneInfo("America/New_York")).time()):
    """Check if current time is during dinner"""
    now = now or datetime.now(ZoneInfo("America/New_York")).time()
...
def dinner_time(start=time(16, 30), end=time(23, 59), now=datetime.now(ZoneInfo("America/New_York")).time()):
    """Check if current time is during dinner"""
    now = now or datetime.now(ZoneInfo("America/New_York")).time()
    # handles ranges that do not cross midnight
    if start <= end:
